...
class Task(Base):
    ...
    frequency = relationship('Frequency', backref='task', uselist=False,
...
class Frequency(Base):
    ...
    id = Column(Integer, primary_key=True)
    type = Column(Enum(TaskFrequencyEnum), default=TaskFrequencyEnum.DAILY,
                  nullable=False)
...
class DailyFrequency(Frequency):
    ...
    id = Column(Integer, ForeignKey('frequencies.id', ondelete="cascade"),
                primary_key=True)
...
class Execution(Base):
    ...
    id = Column(Integer, primary_key=True)
    task_id = Column(Integer, ForeignKey('tasks.id', ondelete='RESTRICT'),
                     nullable=False)
    executed_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    execution_window_id = Column(Integer, ForeignKey('execution_windows.id',
                                 ondelete='RESTRICT'), nullable=True)


class ExecutionWindow(Base):
    """
    This fellow represents a period of time during which a task should be
    executed.
    """
    ...
    status = Column(Enum(ExecutionWindowStatusEnum),
                    default=ExecutionWindowStatusEnum.OPEN, nullable=False)
